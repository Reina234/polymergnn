...
def stack_tensor(tensor, num_samples):

    return tensor.repeat(num_samples, 1)


def generate_out(num_samples):

    return monomer_out.repeat(num_samples * 2, 1)
...
def model_wrapper(perturbed_inputs):
    ...
    mpnn_out = generate_out(num_samples)
...
shap_values = explainer.shap_values(shap_input, n_samples=50)
...
shap_aggregated_2d = np.concatenate(
    [
        ...
        shap_values_np[:, 18:].squeeze().T,  # Polymer Features
    ],
    axis=1,
)
