...
shap_values_evt = shap_evt
shap_values_type = shap_type
...
# Ensure SHAP values are multi-class and select a valid class
if isinstance(shap_values_evt, list) and len(shap_values_evt) > 1:
    shap_values_evt = shap_values_evt[1]  # Select solvation class SHAP values
if isinstance(shap_values_type, list) and len(shap_values_type) > 1:
    shap_values_type = shap_values_type[1]  # Select interaction/solvation class

# Ensure SHAP values exist before plotting
if shap_values_evt is not None and np.array(shap_values_evt).size > 0:
    print("✅ Generating Layered Violin Plot for Stage 1...")
    shap.plots.violin(
        shap_values_evt,
        features=X_train,
    ...
    print("⚠️ Warning: SHAP values are empty for Stage 1! Skipping violin plot.")

if shap_values_type is not None and np.array(shap_values_type).size > 0:
    print("✅ Generating Layered Violin Plot for Stage 2...")
    shap.plots.violin(
        shap_values_type,
        features=X_train2,
    ...
    print("⚠️ Warning: SHAP values are empty for Stage 2! Skipping violin plot.")
...
if isinstance(shap_values_evt, list) and len(shap_values_evt) > 1:
    shap_values_evt = shap_values_evt[1]  # Selecting a valid class
if isinstance(shap_values_type, list) and len(shap_values_type) > 1:
    shap_values_type = shap_values_type[1]  # Selecting interaction/solvation class
...
if shap_values_evt is not None and np.array(shap_values_evt).size > 0:
    ...
    print("⚠️ Warning: SHAP values are empty! Skipping violin plot.")

if shap_values_type is not None and np.array(shap_values_type).size > 0:
    shap.plots.violin(
        shap_values_type, features=X_train2, feature_names=X_train2.columns
    )
else:
    print("⚠️ Warning: SHAP values are empty! Skipping violin plot.")
shap.plots.heatmap(shap_evt)
# Violin plot for Stage 2 (Conditional: Interaction vs Full Solvation)
shap.plots.violin(
    shap_values=shap_type,  # SHAP values for second-stage model
    features=X_train2,  # Feature matrix for conditional model
    feature_names=X_train2.columns,  # Feature names
    plot_type="layered_violin",
)

sample_index = 0  # Choose an index of interest

# Waterfall plot for Stage 1 (Solvation Event)
shap.plots.waterfall(
    explainer_evt(X_test.iloc[sample_index])  # Explains the first-stage prediction
)

# If sample is predicted as solvable, show second-stage Waterfall plot
if model_evt.predict(X_test.iloc[[sample_index]])[0] == 1:
    shap.plots.waterfall(
        explainer_type(X_test2.iloc[sample_index])  # Explains second-stage prediction
    )

shap.plots.waterfall(explainer_evt(X_test.iloc[sample_index]))

shap.plots.waterfall(explainer_type(X_test2.iloc[sample_index]))
