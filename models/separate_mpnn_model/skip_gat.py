...
class SkipGatedGATModule(nn.Module):
    def __init__(
        self,
        ...
        # Attention pooling for solvent and monomer embeddings
        self.monomer_attention_pooling = GlobalAttention(
            gate_nn=nn.Sequential(
                nn.Linear(input_dim + 2, 1), nn.Sigmoid()  # Gate network
            )
        )
        self.solvent_pooling = global_mean_pool  # Simpler pooling for solvent nodes
    ...
    def forward(self, batch):
        ...
        # Pool separately
        monomer_pooled = self.monomer_attention_pooling(
            monomer_embeddings, polymer_mapping[monomer_mask]
        )
        solvent_pooled = self.solvent_pooling(
