...
class ChembertaFusionMoleculeEmbeddingModel(nn.Module):
    def __init__(
        self,
        ...
        projection_ratio: float = 0.3,  # Ratio of MPNN size for RDKit projection
    ):
        super().__init__()
        ...
        self.projection_dim = int(self.mpnn_dim * projection_ratio)
        self.rdkit_projection = nn.Linear(self.rdkit_dim, self.projection_dim)
        self.chemberta_projection = nn.Linear(chemberta_dim, self.projection_dim)
        # Layer Normalization for stability
        self.mpnn_norm = nn.LayerNorm(self.mpnn_dim)
        self.rdkit_norm = nn.LayerNorm(self.projection_dim)
        self.chemberta_norm = nn.LayerNorm(self.projection_dim)
        # Fusion Layer
        fused_dim = self.mpnn_dim + self.projection_dim * 2
    ...
    def forward(self, molgraphs, rdkit_tensor, chemberta_tensor):
        """
        ...
        projected_chemberta = self.chemberta_projection(chemberta_tensor)
        projected_chemberta = self.chemberta_norm(projected_chemberta)
        projected_chemberta = projected_chemberta.squeeze(1)
        # Concatenate features
        fused_input = torch.cat(
            [mpnn_out, projected_rdkit, projected_chemberta], dim=-1
        )
        # fused_input = torch.cat([mpnn_out, fused_input], dim=-1)
