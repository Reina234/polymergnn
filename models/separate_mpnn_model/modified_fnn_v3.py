...
class NewConfigFNNV2(nn.Module):
    def __init__(
        self,
        ...
        self.log_diffusion_head = nn.Sequential(
            nn.Linear(input_dim_fnn, hidden_dim),
            nn.SiLU(),
    ...
    def forward(self, batch):
        ...
        sasa_input = torch.cat([shared_repr, log_rg[:, 0].unsqueeze(-1)], dim=-1)
        sasa = self.sasa_head(sasa_input)  # [B, 2] (mean, std)

        # diff_input = torch.cat([combined_input, log_rg[:, 1].unsqueeze(-1)], dim=-1)

        log_diffusion = self.log_diffusion_head(combined_input)  # [B, 1]
        # log_diffusion = self.log_diffusion_head(shared_repr)
        log_ree_input = torch.cat([shared_repr, sasa.unsqueeze(-1)], dim=-1)
